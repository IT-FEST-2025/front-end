...
`
  ...
  const profileImgSrc = useState(getProfileImageUrl(user?.profilePicture))
  const hasProfilePicture = user?.profilePicture && user.profilePicture !== ""; // Check if there's an actual image
                ...
                {hasProfilePicture ? ( // Kondisional untuk menampilkan gambar atau teks "pp"
                  ...
                  <div
                    className={`w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden cursor-pointer transition-opacity text-2xl font-bold ${
                        shouldUseRedBackground ? "text-white border-2 border-white" : "text-gray-600 border-2 border-[#ff3131]"
                    }`}
                  >
                    pp
                  </div>
                    ...
                    <div className="px-4 py-3 border-b border-gray-200">
                      <div className="flex items-center space-x-3">
                        {hasProfilePicture ? ( // Kondisional yang sama di dalam dropdown
                          ...
                          <div
                            className="w-12 h-12 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden cursor-pointer text-2xl font-bold text-gray-600"
                          >
                            pp
                          </div>
                        )}
                        <div>
                          <p className="text-sm font-medium text-gray-900">{user?.username || "Pengguna"}</p>
